...
public class AbilityDetailsDisplay : MonoBehaviour
{
	...
	Character m_pokemon;
	AbilityDetailsController m_controller;
	...
	void Awake()
	{
		...
		m_physicalTypeTexture = Resources.Load<Texture2D>("Textures/Ability/physical");
	...
	public void SetController(AbilityDetailsController controller)
	{
	...
	public int m_abilityNameX = 50;
	public int m_abilityNameY = 5;
	...
	void OnGUI()
	{
		...
		GUIUtils.DrawGroup(ScreenCoords.BottomScreen, delegate(Rect bounds)
		{
			...
			GUIUtils.DrawGroup(boxBounds, delegate()
			{
				...
				GUI.Label(new Rect(m_abilityNameX, m_abilityNameY, 100, typeBounds.y), "Leech Seed", m_textStyle);
				TypeTag.Display(new Rect(m_tagX, m_abilityNameY, typeBounds.x, typeBounds.y), Abilities.BattleType.Grass);
				GUI.Label(new Rect(m_ppX, m_abilityNameY, 100, typeBounds.y), "PP", m_textStyle);
				GUI.Label(new Rect(boxBounds.width - m_numXPadding - m_numFieldWidth, m_abilityNameY, m_numFieldWidth, typeBounds.y), "0/10", m_numStyle);
