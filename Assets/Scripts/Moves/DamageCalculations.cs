...
public class DamageCalculations
{
	...
	private static bool WillCrit(DamageMove move, Character attacker, Random generator)
	{
		int crit_stage = 0;
		crit_stage += move.CritStage;
	...
	// Damage is equivalent to the move's power
	public static DamageResult FixedDamageFormula(Character attacker, Character defender, DamageMove move, IMoveImpl parentMove)
	{
		DamageResult result = new DamageResult();
		
		// fixed damage is not affected by any multiplier UNLESS the target is immune
		double effectiveness_multiplier = getDamageMultiplier(move.BattleType, defender.Types);
		effectiveness_multiplier = (effectiveness_multiplier == 0) ? 0 : 1.0;
		result.effectiveness = GetEffectiveness(effectiveness_multiplier);
		
		result.amount = (int)(move.Power(attacker, defender, move, parentMove) * effectiveness_multiplier);
		
		return result;
	}
	
	public static DamageResult CalculateDamage(Character attacker, Character defender, DamageMove move, IMoveImpl parentMove)
	{
		return CalculateDamage(attacker, defender, move, parentMove, new Random());
	}
	
	public static DamageResult CalculateDamage(Character attacker, Character defender, DamageMove move, IMoveImpl parentMove, Random generator)
	{
		DamageResult result = new DamageResult();
		
		double same_type_multiplier = attacker.Types.Contains(move.BattleType) ? SAME_TYPE_MULTIPLIER : 1.0;
		
		result.crit = WillCrit(move, attacker, generator);
		...
		uint power = move.Power(attacker, defender, move, parentMove);
		
		result.amount = CalculateBaseDamage(attack, defense, attacker.Level, power, same_type_multiplier, effectiveness_multiplier, crit_multiplier, variance_multiplier);
	...
	public static double getDamageMultiplier(BattleType attackType, List<BattleType> pokemonTypes)
	{
		...
		{
			if (pokemonType == BattleType._None)
			{
				continue;
			}
