...
public class PokemonDefinition
{
	...
	public PokemonDefinition(Pokemon.Species species, BattleType type1, int hp, int atk, int def, int spAtk, int spDef, int spd, List<string> abilities, float maleRatio)
	: this(species, type1, BattleType._None, hp, atk, def, spAtk, spDef, spd, abilities, maleRatio)
	{
	}
	
	public PokemonDefinition(Pokemon.Species species, BattleType type1, BattleType type2, int hp, int atk, int def, int spAtk, int spDef, int spd, List<string> abilities, float maleRatio)
	{
		m_species = species;
	...
	static Dictionary<Pokemon.Species, PokemonDefinition> m_definitions;
	static void Init()
	{
		if (m_isInit)
		{
		...
		m_definitions = new Dictionary<Pokemon.Species, PokemonDefinition>();
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Chespin, BattleType.Grass, 56, 61, 65, 48, 45, 38, abilities, 87.5f));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Quilladin, BattleType.Grass, 61, 78, 95, 56, 58, 57, abilities, 87.5f));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Chesnaught, BattleType.Grass, BattleType.Fight, 88, 107, 122, 74, 75, 64, abilities, 87.5f));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Fennekin, BattleType.Fire, 40, 45, 40, 62, 60, 60, abilities, 87.5f));
		
		// Braixen
		abilities = new List<string> {"Scratch", "Tail Whip", "Ember", "Howl", "Flame Charge", "Psybeam", "Fire Spin", "Lucky Chant", "Light Screen", "Psyshock", "Flamethrower",
			"Will-O-Wisp", "Psychic", "Sunny Day", "Magic Room", "Fire Blast" };
		AddDefinition(new PokemonDefinition(Pokemon.Species.Braixen, BattleType.Fire, 59, 59, 58, 90, 70, 73, abilities, 87.5f));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Delphox, BattleType.Fire, BattleType.Psychic, 75, 69, 72, 114, 100, 104, abilities, 87.5f));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Froakie, BattleType.Water, 41, 56, 40, 62, 44, 71, abilities, 87.5f));
		
		// Frogadier
		abilities = new List<string> {"Pound", "Growl", "Bubble", "Quick Attack", "Lick", "Water Pulse", "Smokescreen", "Round", "Fling", "Smack Down", "Substitute", "Bounce",
			"Double Team", "Hydro Pump"};
		AddDefinition(new PokemonDefinition(Pokemon.Species.Frogadier, BattleType.Water, 54, 63, 52, 83, 56, 97, abilities, 87.5f));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Greninja, BattleType.Water, BattleType.Dark, 72, 95, 67, 103, 71, 122, abilities, 87.5f));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Bunnelby, BattleType.Normal, 38, 36, 38, 32, 36, 57, abilities, 50));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Diggersby, BattleType.Normal, BattleType.Ground, 85, 56, 77, 50, 77, 78, abilities, 50));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Zigzagoon, BattleType.Normal, 38, 30, 41, 30, 41, 60, abilities, 50));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Bulbasaur, BattleType.Grass, BattleType.Poison, 45, 49, 49, 65, 65, 45, abilities, 87.5f));
		...
		abilities = new List<string> {"Scratch", "Growl", "Ember", "Smokescreen", "Dragon Rage", "Scary Face", "Fire Fang", "Flame Burst", "Slash", "Flamethrower", "Fire Spin",
			"Inferno"};
		AddDefinition(new PokemonDefinition(Pokemon.Species.Charmander, BattleType.Fire, 39, 52, 43, 60, 50, 65, abilities, 87.5f));
		
		// Magikarp
		abilities = new List<string> {"Splash", "Tackle", "Flail"};
		AddDefinition(new PokemonDefinition(Pokemon.Species.Magikarp, BattleType.Water, 20, 10, 55, 15, 20, 80, abilities, 50));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Pikachu, BattleType.Electric, 35, 55, 30, 50, 40, 90, abilities, 50));
		...
		AddDefinition(new PokemonDefinition(Pokemon.Species.Squirtle, BattleType.Water, 44, 48, 65, 50, 64, 43, abilities, 87.5f));
		m_isInit = true;
	}
	...
	public static PokemonDefinition GetEntry(Pokemon.Species species)
	{
		Init();
